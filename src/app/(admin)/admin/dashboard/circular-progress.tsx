...
export const CircularProgress: React.FC<CircularProgressProps> = ({
  ...
  const [processedColorConfig, setProcessedColorConfig] = useState<ColorConfigItem[]>([])
  const [currentSegments, setCurrentSegments] = useState<Array<ColorConfigItem & { currentArc: number }>>([])
        ...
        timeouts.push(
          setTimeout(() => {
            setCurrentSegments(prevSegments =>
              prevSegments.map(s => (s.id === segment.id ? { ...s, currentArc: (segment.percentage / 100) * circumference } : s))
            )
          }, delay)
        )
  ...
  const handleSegmentMouseEnter = (segment: ColorConfigItem, event: React.MouseEvent) => {
    ...
    if (currentSegments.length === 0) {
      ...
      return (
        <>
          {gradient && (
            <defs>
              <linearGradient id={singleGradientId} x1="0%" y1="0%" x2="100%" y2="0%">
    ...
    return (
      <>
        <defs>
          {currentSegments.map(segment => {
            if (segment.gradient) {
              ...
              return (
                <linearGradient key={segmentGradientId} id={segmentGradientId} x1="0%" y1="0%" x2="100%" y2="0%">
        ...
        {currentSegments.map(segment => {
          ...
          const segmentEndAngle = segmentStartAngle + (segment.percentage / 100) * 360
          ...
          return (
            <g key={segment.id}>
              {/* Visible Arc */}
              <path
                d={describeArc(size / 2, size / 2, radius, segmentStartAngle - 90, segmentEndAngle - 90)}
                style={{
                  transition: `stroke-dasharray ${animationDuration * 0.6}ms ease-out`,
                  pointerEvents: 'none'
                }}
                stroke={segment.gradient ? `url(#${segmentGradientId})` : segment.color}
                fill="transparent"
                strokeWidth={strokeWidth}
  ...
  return (
    <>
      <div className="relative inline-flex items-center justify-center" style={{ width: size, height: size }}>
        <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} role="img" aria-label="Circular progress chart">
          {enableShadow && (
            <defs>
              <filter id={`shadow-${gradientIdBase}`} x="-50%" y="-50%" width="200%" height="200%">
        ...
        {showText ? (
              ...
              {textValue !== undefined
                ...
                : `${Math.round(
                    currentSegments.length > 0
                      ? currentSegments.reduce((acc, s) => acc + (s.currentArc / circumference) * 100, 0)
                      : currentPercentage
                  )}%`}
            </span>
          </div>
          ...
          <div>
            {data && (
              ...
              >
                <p className="border-b-1">Client: {formatPrice(data.buyers.count)} </p>
                <p>Talent: {formatPrice(data.sellers.count)} </p>
              </div>
            )}
          </div>
        )}
